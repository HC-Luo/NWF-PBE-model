...
pic = ax.contour(lons, lats, zc.T,linewidths = 2,levels = np.arange(5100,5940+40,40),cmap='rainbow')
plt.colorbar(pic,shrink = 0.7)
#wind=ax.quiver(lons,lats,uc.T,vc.T,width=0.002,headwidth=2,headlength=4,pivot='middle')
wind = ax.barbs(lons,lats,uc.T,vc.T,length=6, pivot='middle',barbcolor='k', flagcolor='w', barb_increments=dict(half=2, full=4, flag=20),flip_barb=True)
plt.savefig('/Users/leonidas/AnacondaProjects/sx/NWPsx/After2_NWP.pdf')
plt.show()
